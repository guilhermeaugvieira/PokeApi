...
export class MovesUseCase implements IMoves {
  execute = async (resultPokeApi: IResponse_Pokemon, registroPokemon: Pokemons): Promise<void> => {    
    for(const moveSelected of resultPokeApi.moves){
      ...
      let savePokemonMove: Pokemons_Moves;
      const countPokemonMoves = await getRepository(Pokemons_Moves).count({where: {move: registroMove, pokemon: registroPokemon}});
      if(countPokemonMoves === 0) savePokemonMove = await getRepository(Pokemons_Moves).save(registroPokemonMove);
      else savePokemonMove = await getRepository(Pokemons_Moves).findOne({where: {move: registroMove, pokemon: registroPokemon}});   
      ...
      for(const vgd_result of moveSelected.version_group_details){
        ...
        if(countMoveLearnMethod === 0) await getRepository(MoveLearnMethod).save(registroMoveLearnMethod);
